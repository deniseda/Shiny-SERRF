...
### shiny is a library for web-application
...
server = shinyServer(function(input, output) {
	...
	result_text <- eventReactive(input$go,{
		...
		### remove outlier, the output is a list of values without outlier
		...
		if (!is_example) {     
				...
				withProgress(message = "Normalization in Progress.", value = 0, {
					...
					for (j in 1 : nrow(all)) {
						...
						for (b in 1 : length(levels(batch.))) {
							...
							good_column = apply(train_data_x, 2, function(x) 
							{
								sum(is.na(x)) == 0
							}) & apply(test_data_x, 2, function(x)
							{
								sum(is.na(x)) == 0
							})
			...
			for (i in 1 : length(normalized_dataset)) {	
				if (class(normalized_dataset[[i]])[1] == 'simpleError') {
					cat(paste0("Error! method: ", names(normalized_dataset)[i], ": ",normalized_dataset[[i]]$message,'\n'))
